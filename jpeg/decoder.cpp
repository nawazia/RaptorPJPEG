//
// Created by Ibrahim Nawaz on 10/01/2025.
//
#include <iostream>
#include <iomanip>
#include <fstream>
...
public:
    ...
    uint readBit() {
        ...
        const uint bit = (nextByte >> (7 - nextBit)) & 1;
        nextBit = (nextBit + 1) % 8;
        return bit;
    }
    ...
    uint64_t getByteIndex() {
        const std::streampos currentPosition = inFile.tellg();
        return static_cast<uint64_t>(currentPosition);;
    }

    void seek(const uint64_t pos) {
        inFile.seekg(static_cast<int>(pos), std::ios::beg);
    }
};
...
void readStartOfFrame(BitReader& bitReader, JPGImage* const image) {
    ...
    for (uint i = 0; i < image->numComponents; ++i) {
        ...
        // usually always 1,2,3 but can be 0,1,2 sometimes.
        // force all variants into 1,2,3 for simplicity
        ...
        if (componentID == 1) {
            ...
            // use Y sampling factors as image-wide sampling factor.
...
void readQuantizationTable(BitReader& bitReader, JPGImage* const image) {
    ...
    // underflow risk therefore signed int
    ...
    while (length > 0) {
        ...
        length -= 1;
        // lower nibble via binary ANDing
...
void generateCodes(HuffmanTable& hTable) {
    ...
    for (uint i = 0; i < 16; ++i) {
        ...
        // add 0 to right end of code candidate
        code <<= 1;
    }
}
...
void readStartOfScan(BitReader& bitReader, JPGImage* const image) {
    ...
    image->startOfSelection = bitReader.readByte(); // baseline = 0
    image->endOfSelection = bitReader.readByte();   // baseline = 63, inclusive
...
void printFrameInfo(const JPGImage* const image) {
    ...
    std::cout << "Block Height: " << image->blockHeight << '\n';
    std::cout << "Block Width: " << image->blockWidth << '\n';
...
void readFrameHeader(BitReader& bitReader, JPGImage* const image) {
    ...
    // (continue processing bytes in file)
    while (image->valid) {
...
int base8to10_v2(const std::vector<int>& baseEight) {
    std::vector<int> powers(baseEight.size());
    int power = 0;
    for (int i = powers.size() - 1; i >= 0; --i) {
        powers[i] = std::pow(8, power);
        power++;
    }
    return std::inner_product(baseEight.begin(), baseEight.end(), powers.begin(), 0);
}

std::vector<int> findRIs(BitReader& bitReader, const byte prevRI) {
    const uint64_t currentPosition = bitReader.getByteIndex();
    std::vector result(1, RSTMap.find(prevRI)->second);
    byte last = bitReader.readByte();
    byte current = bitReader.readByte();
    while (last == 0xFF && (current >= RST0 && current <= RST7)) {
        result.push_back(RSTMap.find(current)->second);
        last = bitReader.readByte();
        current = bitReader.readByte();
    }

    bitReader.seek(currentPosition);
    return result;
}

static void checkValidScan(BitReader& bitReader, JPGImage* image) {
    image->validBits.clear();
    const uint64_t curPos = bitReader.getByteIndex();
    ...
    while (true) {
        uint64_t byteIndex = bitReader.getByteIndex() - 2; // Current position of the 2-byte marker
        int mappingIndex = static_cast<int>(byteIndex / image->chunkSize) + 1;

        // Search for SOS, DHT or EOI. End of scan will always be visible if patched.jpg
        if (last == 0xFF && (current == SOS || current == DHT || current == EOI)) {
            if (inValidSpan) {
                image->validBits.push_back({static_cast<uint32_t>(spanStart), static_cast<uint32_t>(byteIndex), static_cast<unsigned>(base8to10_v2(lastRI) + 1)});
            }
            image->validityUpperBound = byteIndex;
            break;
        }

        // Check if the current byte index maps to a valid entry
        if (image->mapping[mappingIndex] != -1) {
            // start of scan
            if (curPos == byteIndex) {
                inValidSpan = true;
                spanStart = byteIndex;
            }
            // Check for a valid 2-byte marker (FFD0-FFD7)
            if (last == 0xFF && (current >= RST0 && current <= RST7)) {
                if (!inValidSpan) {
                    if (std::vector<int> currRI = findRIs(bitReader, current); base8to10_v2(lastRI) < base8to10_v2(currRI)) {
                        // Start a new valid span
                        inValidSpan = true;
                        spanStart = byteIndex;
                        lastRI = currRI;
                    }
            ...
            // Finalize the last span if it was still open
            if (inValidSpan) {
                image->validBits.push_back({static_cast<uint32_t>(spanStart), static_cast<uint32_t>(byteIndex), static_cast<unsigned>(base8to10_v2(lastRI) + 1)});
                inValidSpan = false;
            }
        ...
        // Update last and current bytes
        last = current;
        current = bitReader.readByte();
    }

    // Set image->validScan based on whether any valid spans were found
    image->validScan = !image->validBits.empty();

    // Reset the bit reader to the original position
    bitReader.seek(curPos);
}

void decodeHuffmanData(BitReader& bitReader, JPGImage* image, bool findRI = false);
void decodeHuffmanData2(BitReader& bitReader, JPGImage* image);
void writeBMP(const JPGImage* image, const std::string& filename);
...
void readScans(BitReader& bitReader, JPGImage* const image) {
    ...
    // check for partial decoding
    // decodeHuffmanMethod: 0 = 0 negatives, full recovery
    //                      1 = >0 negatives, partial recovery
    //                      2 = findRI method.
    int decodeHuffmanMethod;
    if (image->mapping.empty()) {
        decodeHuffmanMethod = 2;
    } else {
        if (std::any_of(image->mapping.begin(), image->mapping.end(), [](int x){ return x == -1; })) {
            decodeHuffmanMethod = 1;
        } else {
            decodeHuffmanMethod = 0;
        }
    }

    if (decodeHuffmanMethod == 0) {
        decodeHuffmanData(bitReader, image);
    } else if (decodeHuffmanMethod == 1) {
        decodeHuffmanData2(bitReader, image);
    } else {
        // decodeHuffmanMethod == 2
        decodeHuffmanData(bitReader, image, true);
        return;
    }
    ...
    // TODO: output DC image.
    // const std::string outpath = "/Users/i/Downloads/DNA_STorage/code/jpeg/data/0.bmp";
    ...
    while (image->valid) {
        ...
        // end of image - this is the furthest a baseline image can go.
        if (current == EOI) {
            break;
        }
        ...
        // additional scans (progressive only)
            ...
            // const std::string outpath = "/Users/i/Downloads/DNA_STorage/code/jpeg/data/" + std::to_string(counter) + ".bmp";
            // writeBMP(image, outpath);
            // counter++;
        }
        ...
        // ignore multiple 0xFF's in a row
            ...
            std::cout << "Error - Invalid marker: 0x" << std::hex << (uint)current << std::dec << " at " << bitReader.getByteIndex() << '\n';
            image->valid = false;
            return;
        }
        last = bitReader.readByte();
...
void addMapping(JPGImage* const image, const std::vector<std::int32_t>& mapping) {
    image->mapping = mapping;
}

void addChunkSize(JPGImage* const image, const int32_t chunk_size) {
    image->chunkSize = chunk_size;
}

JPGImage* readJPG(const std::string& filename, const std::vector<std::int32_t>& mapping, int32_t chunk_size) {
    // open file
    ...
    // check if we are using mapping.
    if (!mapping.empty()) {
        addMapping(image, mapping);
        addChunkSize(image, chunk_size);
    }
    readFrameHeader(bitReader, image);
    printFrameInfo(image);

    // after SOS break, we are here. i.e. Marker FFDA just popped.
    if (!image->valid) {
        return image;
    }
...
byte getNextSymbol(BitReader& bitReader, const HuffmanTable& hTable) {
    uint currentCode = 0;
    // 0-15 = all possible code lengths
    for (uint i = 0; i < 16; ++i) {
        int bit = bitReader.readBit();
        // byte output converts -1 to 255, which is not a valid symbol. HACK
        if (bit == -1) {
            return -1;
        }
        // grow Huffman code, until matches a code in Huffman table
...
bool decodeBlockComponent(
    ...
    BitReader& bitReader,
    int* const component, // image->blocks[(y+v)*image->blockWidthReal+(x+h)][i] = image->block[blockIdx][component]
    int& previousDC,
    uint& skips,          // 0 if !successive_approximation
    ...
    if (image->frameType == SOF0) {
        ...
        // coeff < 2^(length - 1). e.g. length = 3, 2^(3-1)=4. This is the midpoint.
        // Anything below this is negative, as prepended zeroes in the bitstream are only used for negative.
        // Subtract 2^length - 1. e.g. length = 3, subtract 7.
        ...
        // handle remaining 63 coeff, all AC.
        for (uint i = 1; i < 64; ++i) {
        ...
        if (image->startOfSelection == 0 && image->successiveApproximationHigh == 0) {
            ...
            // invert to -ve if in "first-half"
            if (length != 0 && coeff < (1 << (length - 1))) {
                coeff -= (1 << length) - 1;
            }
            // add previousDC, iterate previousDC
            coeff += previousDC;
            previousDC = coeff;
            // add component at 0th index = DC.
            ...
            if (skips == 0) {
                for (; i <= image->endOfSelection; ++i) {
                    ...
                    byte numZeroes = symbol >> 4;
                    byte coeffLength = symbol & 0x0F; // should be 0 or 1
                    int coeff = 0;
                    ...
                    if (coeffLength != 0) {
                        ...
                        // traverse zigzagmap for skips + extra skips (= skips).
                        if (numZeroes != 15) {
                            skips = 1 << numZeroes;
                            uint extraSkips = bitReader.readBits(numZeroes);
                            if (extraSkips == (uint)-1) {
                    ...
                    // when we leave the while loop, we are at the index of coeff.
                    if (i < 64) {
                        component[zigZagMap[i]] = coeff;
                    }
                }
...
// decode all the Huffman data and fill all MCUs for a scan. Called immediately after FFDA (SOS)
void decodeHuffmanData(BitReader& bitReader, JPGImage* const image, bool findRI) {
    const uint64_t startIdx = bitReader.getByteIndex();
    int previousDCs[3] = { 0 };
    uint skips = 0;
    ...
    // decode one full MCU.
    // In case of subsampling, one MCU can be 8x8, 16x8, 8x16, 16x16.
    // In case of subsampling, we may need to pad. Therefore, use image height/width, not real.
    // Pad will be accounted for with v, h loop.
    for (uint y = 0; y < image->blockHeight; y += yStep) {
        for (uint x = 0; x < image->blockWidth; x += xStep) {
            if (restartInterval != 0 && (y * image->blockWidthReal + x) % restartInterval == 0) {
                // reset DC differential encoding for all components
                previousDCs[0] = 0;
                previousDCs[1] = 0;
                previousDCs[2] = 0;
                // skips = 0 for successive approximation
                skips = 0;
                // move to next byte if in the middle of byte.
                bitReader.align();
            }
    ...
    // image->blocks filled with quantized DCT coefficients
    if (findRI) {
        const uint64_t bytesRead = bitReader.getByteIndex() - startIdx;
        const uint64_t totalBlocks = image->blockWidth * image->blockHeight;
        float avgBlockSize = static_cast<float>(bytesRead) / totalBlocks;

        std::cout << std::fixed << std::setprecision(2);
        std::cout << "Average block size: " << avgBlockSize << " bytes\n";
        image->avgBlockSize = avgBlockSize;
    }
}

void decodeHuffmanData2(BitReader& bitReader, JPGImage* const image) {
    // check if there is valid in scan
    checkValidScan(bitReader, image);
    if (!image->validScan) {
        std::cout << "Error - no valid data in scan\n";
        bitReader.seek(image->validityUpperBound);
        return;
    }

    // std::cout << "spans: " << std::endl;
    // for (auto span : image->validBits) {
    //     std::cout << span[0] << " " << span[1] << " " << span[2] << std::endl;
    // }

    for (auto span : image->validBits) {
        const uint32_t start = span[0];
        const uint32_t end = span[1];
        const uint32_t offset = span[2] * image->restartInterval;

        // std::cout << "Running decoding at " << start << " to " << end << std::endl;
        int32_t blockCount = 0;
        int previousDCs[3] = { 0 };
        uint skips = 0;

        bitReader.seek(start);
        uint64_t curPos = bitReader.getByteIndex();
        while (curPos < end) {
            if (blockCount % image->restartInterval == 0) {
                previousDCs[0] = 0;
                previousDCs[1] = 0;
                previousDCs[2] = 0;
            ...
            for (uint i = 0; i < image->numComponents; ++i) {
                ...
                if (component.usedInScan) {
                    decodeBlockComponent(
                        image,
                        bitReader,
                        image->blocks[offset + blockCount][i],
                        previousDCs[i],
                        skips,
                        image->huffmanDCTables[component.huffmanDCTableID],
                        image->huffmanACTables[component.huffmanACTableID]);
                    curPos = bitReader.getByteIndex();
                }
            }
            ++blockCount;
        }
    }
    std::cout << bitReader.getByteIndex() << std::endl;
    if (bitReader.getByteIndex() != image->validityUpperBound) {
        std::cout << "Moving index from " << bitReader.getByteIndex() << " to " << image->validityUpperBound << std::endl;
        bitReader.seek(image->validityUpperBound);
    }
}
...
std::string basename(const std::string& path) {
    size_t last_slash = path.find_last_of("/\\"); // Find the last '/' or '\'

    if (last_slash == std::string::npos) {
        return path; // No slash found, return the original path
    } else {
        return path.substr(last_slash + 1); // Return the substring after the last slash
    }
}

std::vector<std::int32_t> readMapping(const std::string& mapping) {
    std::ifstream file(mapping, std::ios::binary); // Open in binary mode

    if (!file.is_open()) {
        std::cerr << "Error: Could not open file." << std::endl;
        return {};
    }
    // 1. Read the shape (you'll need to know this beforehand or store it in the file)
    std::string mappingLen = basename(mapping);
    mappingLen = mappingLen.substr(0, mappingLen.length() - 4);
    std::pair<std::string, std::string> result;
    size_t underscorePos = mappingLen.find('_');

    if (underscorePos != std::string::npos) {
        result.first = mappingLen.substr(0, underscorePos);
        result.second = mappingLen.substr(underscorePos + 1);
    }
    int array_length = std::stoi(result.first);
    int32_t chunk_size = std::stoi(result.second);

    // 2. Read the data type (important to know this too)
    // NumPy's int32 corresponds to std::int32_t in C++
    std::vector<std::int32_t> data(array_length);  // Vector to store the data

    // 3. Read the array data
    file.read(reinterpret_cast<char*>(data.data()), data.size() * sizeof(std::int32_t));

    if (file.gcount() != data.size() * sizeof(std::int32_t)) {
        std::cerr << "Error: Could not read all data from file." << std::endl;
        return {};
    }

    file.close();

    data.push_back(chunk_size);
    return data;
}
...
int main(int argc, char** argv) {
    ...
    if (argc == 3) {
        ...
        chunk_size = mapping.back();
        mapping.pop_back();
        std::cout << "Mapping has size: " << mapping.size() << std::endl;
    } else {
        std::cout << "No mapping provided" << std::endl;
    }
    ...
    // read image
    JPGImage* image = readJPG(filename, mapping, chunk_size);

    // if mapping.empty(), return after calculating avg block bit size
    if (mapping.empty()) {
        // * 100 as we rounded to 2dp. Therefore, avgBlockSize * 100 is an int
        // we can catch exit code (int) in encode.py and divide back by 100 to get float.
        return (image->avgBlockSize)*100;
    }
