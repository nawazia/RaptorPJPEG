...
from norec4dna.helper import invalidPriority
...
class RU10Encoder(Encoder):
    ...
    def set_priority_chunks(self, priority_chunks):
        """
        Sets the priority chunks for the encoder. The encoder will encode chunks with index < priority_chunks as high priority.
        :param priority_chunks: List of chunks to prioritize
        :return:
        """
    ...
    def do_encode(self):
        ...
        def _encode(pseudo, count=None):
            ...
            if new_pack.degree == 2:
                count += 1
            # add new encoded packet (RU10Packet) to encodedPackets
            self.encodedPackets.add(new_pack)
            if not self.oligoLen:
                self.oligoLen = len(new_pack.get_dna_struct(False))
        ...
        if self.pseudo_decoder is not None:
            ...
            while (self.number_of_packets_encoded_already() < (self.number_of_chunks + (
                ...
                # This process continues until the receiver signals that the
                # message has been received and successfully decoded.
                _encode(pseudo=False, count=count)
    ...
    def create_new_packet(self, systematic: bool = False, seed: typing.Optional[int] = None) -> RU10Packet:
        ...
        # sorted indices of chunks to choose from l
    ...
    def generate_intermediate_blocks(self) -> typing.List[bytes]:
        ...
        # Each source symbol contributes to three LDPC symbols:
        # The LDPC symbol at indices: 1. b; 2. (b + a) % s; 3. (b + 2a) % s
        # The indices of the source symbols contributing to each LDPC symbol are stored in compositions.
        ...
        # For each LDPC symbol, XOR the source symbols at the indices stored in compositions[i].
        # Append the resulting LDPC symbol to self.chunks.
        for i in range(0, s):
        ...
        # Prepare building Half symbols by building Gray sequence of length k + s
        hprime = int(ceil(np.float64(h) / 2))
        m = buildGraySequence(k + s, hprime)

        # self.chunks = source + LDPC
        # Create h Half symbols by combining self.chunks (source and LDPC symbols) using XOR operations.
        # For each Half symbol, XOR intermediates indicated in Gray sequence.
        for i in range(0, h):
    ...
    def save_config_file(self, default_map=None, section_name=None, add_dot_fasta=False):
        ...
        config_file_name = "{}.ini".format(self.file)
