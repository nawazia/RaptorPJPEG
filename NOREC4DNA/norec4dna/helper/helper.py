...
def readMarkers(file, dropFFDX=False):
    ...
    while True:
        ...
        if not byte:
            break
        originalFile.append(byte)

        if byte == b'\xff':
            current_position = file.tell()  # Store current position
            next_byte = file.read(1)

            if next_byte and next_byte != b'\x00':  # Check or EOF
                if dropFFDX:
                    if b'\xd0' <= next_byte <= b'\xd7':
                        pass
                    else:
                        locations.append((offset, next_byte))
                else:
                    locations.append((offset, next_byte))

            if next_byte:
                file.seek(current_position)  # Restore the file pointer

        offset += 1
    return locations, originalFile

def invalidPriority(packet, count : int, priority_chunks : int = None):
    if priority_chunks is None:
        # default = no priority
        return False
    if packet.degree == 1:
        # check chunk comes from high priority chunks
        if (packet.used_packets[0] > priority_chunks):
            return True
    elif packet.degree == 2:
        if count < priority_chunks / 2:
            # check chunk comes from high priority chunk
            for chunkIdx in packet.used_packets:
                if chunkIdx > priority_chunks:
                    return True
    return False
